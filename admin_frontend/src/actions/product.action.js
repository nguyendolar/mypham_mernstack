...
export const getBill = (status, page) => async (dispatch, getState) => {
    console.log("nha", status);
    console.log("page", getState().productReducers.author.page)
    let link = "http://localhost:8080/bill/status/99/" + page
    if (status === "0")
    {
        link = "http://localhost:8080/bill/status/0/" + page
    }
    if (status === "1")
    {
        link = "http://localhost:8080/bill/status/1/" + page
    }
    let res = null
    try
    {
